...
sidewalkColorMap[Detached_Sidewalk] = "#008000"
...
function drawOnMap(allPaths): any {
  ...
  allPaths.forEach(element => {
    ...
    console.log("placeID: " + element["PlaceID"] + " from: " + element.latitude + " " + element.longitude + " - to:  " + lat_to + " " + lng_to);
    // }
    path.push({
    ...
    let color = getSideWalkClassificationColor(element.features);
...
function drawSidewalkData(mapBounds): any {
  ...
  let url_api = "https://dgv28m9zig.execute-api.us-east-1.amazonaws.com/test?lat_min=" + Math.min(...lats) + "&lat_max=" + Math.max(...lats) + "&lon_min=" + Math.min(...lngs) + "&lon_max=" + Math.max(...lngs);
  console.log(url_api);
...
function initMap() {
  ...
  if(!LAT){
    LAT=33.45190643296234;
  }
  if(!LNG){
    LNG=-112.08846038448033;
  }
